...
const { getScheduleTimes } = require('../../tribunes/scheduleTimes');

function createPartButtons() {
    const buttons = ['1','2'].map(p =>
        new ButtonBuilder().setCustomId(`part|${p}`).setLabel(`${p} часть`).setStyle(ButtonStyle.Secondary)
    );
...
function createUnregisterButton(day, part, role, userId) {
    try {
        ...
        const timeStr = getScheduleTimes()[dayName]?.[part] || '00:00';
        const date = new Date(`${dateStr}T${timeStr}:00`);
        const now = new Date();
        const diffMs = date - now;
        if (diffMs < 10800000) return [];
        return [new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`unreg|${day}|${part}|${role}|${userId}`).setLabel('Отписаться').setStyle(ButtonStyle.Secondary)
        )];
    } catch { return []; }
}
...
async function handleSignupButtons(interaction) {
    ...
    if (step === 'day') {
        ...
        return interaction.reply({
            ...
            components: createPartButtons(), flags: 64
        });
    }

    if (step === 'part') {
        const session = getUserSession(userId);
        if (!session?.day) return interaction.reply({ embeds: [makeErrorEmbed('Сначала выберите день.')], flags: 64 });
        session.part = value;
        return interaction.reply({
    ...
    if (step === 'role') {
        const session = getUserSession(userId);
        if (!session?.day || !session?.part) return interaction.reply({ embeds: [makeErrorEmbed('Сначала выберите день и часть.')], flags: 64 });

        const data = loadData();
        const { day, part } = session;
        const role = value;

        if (!data[day]) return interaction.reply({ embeds: [makeErrorEmbed('❌ Неверная дата.')], flags: 64 });
        ...
        if (already.includes(userId)) return interaction.reply({ embeds: [makeErrorEmbed('⚠️ Вы уже зарегистрированы на эту роль.')], flags: 64 });
        if (already.length >= maxSlots) return interaction.reply({ embeds: [makeErrorEmbed('❌ Все места заняты.')], flags: 64 });

        const member = await interaction.guild.members.fetch(userId).catch(() => null);
        if (!member) return interaction.reply({ embeds: [makeErrorEmbed('❌ Не удалось получить информацию о вас.')], flags: 64 });

        const isMale = hasGenderRole(member, 'мужская');
        const isFemale = hasGenderRole(member, 'женская');
        if (!isMale && !isFemale) return interaction.reply({ embeds: [makeErrorEmbed('❌ У вас должна быть роль с «мужская» или «женская».')], flags: 64 });
        ...
        if (role === 'ведущий') {
            ...
            if ((hasMale && isMale) || (hasFemale && isFemale)) {
                return interaction.reply({ embeds: [makeErrorEmbed('❌ На роль ведущего нельзя регистрироваться, если уже есть участник того же пола.')], flags: 64 });
            }
        }
        ...
        return interaction.reply({
            ...
            components: unregisterBtn, flags: 64
        });
    }
